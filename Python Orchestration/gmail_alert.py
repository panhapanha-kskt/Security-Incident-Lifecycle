...
def build_html_email(alert: dict) -> str:
    ...
    def kv(label: str, value: str, color: Optional[str] = None) -> str:
        vc = f"color:{color};" if color else "color:#111111;"
        return (
            f"<tr>"
            f'<td style="padding:7px 12px;border-bottom:1px solid #e0e0e0;font-size:10px;'
            f"color:#888;text-transform:uppercase;letter-spacing:.1em;"
            f'font-family:Courier New,monospace;width:130px;vertical-align:top;">{label}</td>'
            f'<td style="padding:7px 12px;border-bottom:1px solid #e0e0e0;font-size:12px;'
    ...
    if srcip != "—":
        vt_link = (
            ...
            f'style="display:inline-block;background:#f0f0f0;border:1px solid #ccc;'
            f"border-radius:4px;padding:5px 10px;font-size:11px;color:#333;"
    ...
    if full_log:
        raw_log_section = (
            ...
            f'color:#888;margin-bottom:7px;">Raw Log (truncated)</div>'
            f'<pre style="background:#f5f5f5;border:1px solid #ddd;border-radius:4px;'
            f"padding:10px;font-family:Courier New,monospace;font-size:10px;color:#333;"
...
<table width="640" cellpadding="0" cellspacing="0"
  style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.15);border:1px solid #ddd;">
  ...
  <tr><td style="background:rgba(0,0,0,.4);padding:5px 22px;">
    <span style="font-size:9px;letter-spacing:.3em;text-transform:uppercase;color:rgba(255,255,255,.5);">
       BLUE TEAM OPERATIONS CENTER &middot; WAZUH SOC DEFENSE PLATFORM
    </span>
  </td></tr>
    ...
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td style="padding:18px 22px 18px;">
          ...
          <div style="font-size:30px;font-weight:700;color:#000000;letter-spacing:.05em;line-height:1;">
            {sev}
          </div>
        </td>
  ...
  <!-- description -->
  <tr><td style="background:#f7f7f7;padding:16px 22px;border-bottom:1px solid #e0e0e0;">
  ...
  <tr><td style="background:#ffffff;padding:22px;">
    ...
    <div style="margin-bottom:18px;">
      <div style="font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:#888;margin-bottom:7px;">MITRE ATT&amp;CK</div>
      {mitre_html}
    </div>

    {raw_log_section}

    <div style="background:#f5f5f5;border:1px solid #ddd;border-radius:4px;padding:12px 14px;margin-bottom:18px;">
      <div style="font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:#888;margin-bottom:8px;">
        Analyst Checklist
      </div>
    ...
    <div style="margin-bottom:20px;">
      ...
      <a href="https://attack.mitre.org/"
        style="display:inline-block;background:#f0f0f0;border:1px solid #ccc;border-radius:4px;
        padding:5px 10px;font-size:11px;color:#333;text-decoration:none;
        font-family:Courier New,monospace;">&#128737; MITRE Navigator</a>
    </div>

    <div style="border-top:1px solid #ddd;padding-top:14px;font-size:10px;color:#777;text-align:center;line-height:1.8;">
      Auto-generated by <strong style="color:#555;">Wazuh SOC Defense Platform v3</strong>
      &nbsp;&middot;&nbsp; {now_str}<br>
...
class GmailAlerter:
    ...
    def send(self, alert: dict) -> bool:
        ...
        if last is not None and (now - last) < self._dedup_sec:
            return False   # within cooldown window — expected skip
